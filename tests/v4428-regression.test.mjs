...
const css = read('visionqc-v4428-clean.css');

test('v4.4.28 assets and the supplied TOPTEC logo are exact', () => {
  assert.match(html, /VisionQC DirectExport v4\.4\.28/);
  assert.match(html, /assets\/index-v4\.4\.28\.js/);
  assert.match(html, /visionqc-v4428-clean\.css\?v=4\.4\.28/);
  assert.match(html, /visionqc-extension\.js\?v=4\.4\.28/);
  assert.ok(existsSync(resolve(root, 'assets/index-v4.4.28.js')));
...
test('picker gets a long user-controlled window without the former 120 second abort', () => {
  ...
  const browse = js.slice(js.indexOf('async function browseSimulationPath'), js.indexOf('function createPositionKey'));
  assert.match(browse, /timeout:PICKER_TIMEOUT_MS/);
  assert.doesNotMatch(browse, /timeout:120000/);
});
