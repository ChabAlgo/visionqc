...
test('dashboard chart, Position cards, misses and Threshold inputs fit the requested geometry', async ({ page }) => {
  ...
  expect(result.thresholdWidth).toBeGreaterThanOrEqual(47);
  expect(result.thresholdWidth).toBeLessThanOrEqual(49);
...
test('running Simulation progress remains fixed while moving between menus', async ({ page }) => {
  await page.route('**/api/status', route => route.fulfill({ status:200, contentType:'application/json', body:JSON.stringify({ ok:true, agentVersion:'1.3.27', vpdlAvailable:true }) }));
  await open(page);
