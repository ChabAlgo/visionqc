...
test('running Simulation progress remains fixed while moving between menus', async ({ page }) => {
  await page.route('**/api/status', route => route.fulfill({ status:200, contentType:'application/json', body:JSON.stringify({ ok:true, agentVersion:'1.3.34', vpdlAvailable:true }) }));
  await open(page);
