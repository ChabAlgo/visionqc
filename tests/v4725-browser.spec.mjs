...
test('history DB delete requires confirmation and refreshes the empty database',async({page})=>{
 ...
 await page.route('**/api/status',route=>route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({ok:true,agentVersion:'1.3.34',vpdlAvailable:true,running:false})}));
...
test('daily NG chart expands a five percent peak to a zero-to-eight-percent axis',async({page})=>{
 ...
 const labels=await page.locator('.vq43-main-history-dashboard .vq43-history-axis > text').allTextContents();
