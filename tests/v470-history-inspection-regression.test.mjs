...
test('v4.7.7 Web and Agent download targets are aligned', () => {
  assert.match(read('VERSION.txt'), /v4\.7\.7/);
  assert.match(html, /visionqc-extension\.js\?v=4\.7\.7/);
  assert.match(html, /visionqc-v470\.css\?v=4\.7\.7/);
  assert.match(js, /const VERSION = '4\.7\.7'/);
  assert.match(js, /const EXPECTED_AGENT_VERSION = '1\.2\.3'/);
  assert.match(js, /VisionQC_Agent_Installer_v1\.2\.3\.exe/);
  assert.match(js, /VisionQC_Offline_v4\.7\.7\.zip/);
});
