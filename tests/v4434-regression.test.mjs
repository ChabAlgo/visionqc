...
test('v4.8.7 assets and the supplied TOPTEC logo are exact', () => {
  assert.match(html, /VisionQC DirectExport v4\.8\.7/);
  assert.match(html, /assets\/index-v4\.4\.33\.js/);
  assert.match(html, /visionqc-v4433-clean\.css\?v=4\.8\.7/);
  assert.match(html, /visionqc-extension\.js\?v=4\.8\.7/);
...
test('Simulation Start requires preload and rechecks Runtime License', () => {
  ...
  assert.ok(start.indexOf('checkSimulationRuntime') < start.indexOf('await requestSimulationStart('));
});
...
test('Web preload signature matches Agent ordering and Windows path normalization', () => {
  ...
  assert.equal(value, 'green|True|0|P:False|A:False|D:True|M:False|AN_TOP|G:H:\\RUNTIME\\AN.VRWS|CA_TOP|G:H:\\RUNTIME\\CA.VRWS');
});
