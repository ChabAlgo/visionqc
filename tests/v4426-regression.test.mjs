...
const css = read('visionqc-v4426-clean.css');

test('v4.4.26 assets are versioned and loaded in override order', () => {
  assert.match(html, /VisionQC DirectExport v4\.4\.26/);
  assert.match(html, /assets\/index-v4\.4\.26\.js/);
  assert.match(html, /visionqc-extension\.js\?v=4\.4\.26/);
  assert.match(html, /visionqc-v4426-clean\.css\?v=4\.4\.26/);
  assert.ok(html.indexOf('visionqc-v4426-clean.css') > html.indexOf('visionqc-extension.css'));
  assert.ok(existsSync(resolve(root, 'assets/index-v4.4.26.js')));
  assert.ok(existsSync(resolve(root, 'visionqc-v4426-clean.css')));
});
...
test('Simulation Options stays in its FHD grid column and keeps a dedicated stateful scroller', () => {
  ...
  assert.match(css, /\.vq43-sim-options\{[^}]*position:sticky!important/);
  assert.doesNotMatch(css, /\.vq43-sim-options\{[^}]*position:fixed!important/);
...
test('Fallback and Preview no longer require a wide horizontal table', () => {
  ...
  assert.match(css, /\.vq43-fallback-metrics input,\.vq43-fallback-sample input\{[^}]*background:#050d18!important/);
  assert.match(css, /\.vq43-fallback-sample button\{[^}]*height:34px!important/);
...
test('Workspace structure requests are serialized before entering the Agent and cached for navigation', () => {
  assert.match(js, /const workspaceInspectInflight = new Map\(\)/);
  assert.match(js, /let workspaceInspectQueueTail = Promise\.resolve\(\)/);
  assert.match(js, /WORKSPACE_INSPECT_TIMEOUT_MS = 180000/);
  assert.match(js, /WORKSPACE_INSPECT_CACHE_MS = 10 \* 60 \* 1000/);
  assert.match(js, /workspaceInspectQueueTail\.catch\(\(\) => undefined\)\.then/);
  assert.match(js, /workspaceInspectInflight\.get\(requestKey\)/);
  assert.match(js, /timeout:WORKSPACE_INSPECT_TIMEOUT_MS/);
  assert.doesNotMatch(js, /timeout:30000/);
});

test('Workspace picker, queue, reading and final state use one persistent source of truth', () => {
  assert.match(js, /const workspaceInspectStatus = new Map\(\)/);
  assert.match(js, /const workspaceInspectGeneration = new Map\(\)/);
  assert.match(js, /workspacePathMatches\(workspacePathFor\(positionKey, kind\), path\)/);
  assert.match(js, /\['picking','queued','reading'\]\.includes\(status\.phase\)/);
  assert.match(js, /data-vq-workspace-busy=/);
  assert.match(js, /badge:'QUEUED'/);
  assert.match(js, /badge:'READING'/);
  assert.match(js, /error\?\.code === 'REQUEST_TIMEOUT' && attempt === 0/);
  assert.match(js, /normalizeStoredWorkspaceInfo\(p\.greenWorkspaceInfo\)/);
  assert.match(js, /signal is aborted without reason\|aborterror/);
  assert.match(js, /resumeStoredWorkspaceInspections\(\)/);
});

test('Workspace status refresh updates only targeted controls', () => {
  ...
  assert.match(refreshBlock, /data-vq-action="simulation-browse"/);
});

test('Simulation log timestamps are always HH:mm:ss.SSS', () => {
  assert.match(js, /function formatSimulationLogTime\(value\)/);
  assert.match(js, /formatSimulationLogTime\(item\.time\)/);
  assert.match(js, /formatLogTime\(value\) \{ return formatSimulationLogTime\(value\); \}/);
  assert.match(js, /\(\\d\{1,2\}\)시\\s\*\(\\d\{1,2\}\)분/);
  assert.doesNotMatch(js, /toLocaleTimeString/);
  assert.match(css, /\.vq43-sim-log-line\{grid-template-columns:94px 58px minmax\(0,1fr\)!important\}/);
});
