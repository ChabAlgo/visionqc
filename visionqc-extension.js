(() => {
  ...
  const EXPECTED_AGENT_VERSION = '1.4.3';
  const AGENT_INSTALLER_URL = './downloads/VisionQC_Agent_Installer_v1.4.3.exe';
  const OFFLINE_PACKAGE_URL = './downloads/VisionQC_Offline_v4.8.3.zip';
  ...
  const state = {
    ...
    simulationAgent: { status: 'idle', version: '-', vpdlAvailable: null, vpdl: '-', installedVpdl: '-', activeVpdlApiVersion: '-', availableVpdl: [], license: '-', gpu: '-', message: 'Local Agent 연결 전' },
  ...
  function bindPageControls() {
    ...
    $$('[data-chart-position]', shell).forEach(input => {
      input.onchange = async () => {
        if((state.remoteAnalysis||input.dataset.chartScope==='history') && state.simulationAgent.analysisApiVersion<2){input.checked=!input.checked;return showToast('Position 선택에는 새 Agent 업데이트가 필요합니다.',true);}
        const scope=input.dataset.chartScope;
        const selected=$$('[data-chart-position]',shell).filter(el=>el.dataset.chartScope===scope&&el.checked).map(el=>el.dataset.chartPosition);
        if(scope==='history'){
          Object.assign(state.historyFilters,{positions:selected,position:'',workspaceType:'',workspaceKey:'',tool:''});
          await refreshHistory(true);
        }else{
          state.dashboardPositions=selected;
          if(state.remoteAnalysis)await refreshAgentDashboard({actual:false,thresholds:false});else renderCurrentPage();
  ...
  const SIMULATION_TOOLTIPS = {
    ...
    originalProcess:'Green 검사를 독립 프로세스에서 원본 DL_Simulation 방식으로 실행합니다. Runtime File Load는 구성 확인용이며 검사 시 새로 로드합니다. 저장된 Workspace 설정을 그대로 사용하고 TensorRT·메모리 변경 옵션은 적용하지 않습니다. 검사 종료 후 Runtime File Load를 다시 누르세요.',
  ...
  function applySimulationTooltips(root = document) {
    ...
    $$('input,select,textarea,button,label.vq43-sim-check', root).forEach((element) => {
      ...
      const originalGreen = state.simulationMode === 'green' && ensureSimulationForm().green.originalProcess;
      const actionText = action === 'simulation-runtime-load' ? (originalGreen ? '선택한 Workspace 구성을 확인합니다. 검사는 독립 프로세스에서 새 Runtime으로 실행합니다.' : '선택한 Workspace를 VPDL Runtime에 실제로 로드해 Simulation 시작 시 재사용합니다.')
        : action === 'simulation-start' ? (originalGreen ? '독립 Green 프로세스에서 Runtime 생성·Workspace 로드 후 검사를 시작합니다.' : '사전 로드된 Runtime으로 즉시 Simulation을 시작합니다.')
  ...
  function saveAnalysisSnapshot() {
    ...
    const data = structuredClone({ remoteAnalysis:reference,resultInputs:state.resultInputs, ngImages:state.ngImages,
      ...
      thresholds:state.thresholds, dashboardDate:state.dashboardDate, dashboardPositions:state.dashboardPositions, page:state.page });
  ...
  async function chooseAgentResultFiles(position) {
    ...
    try {
      ...
      state.remoteAnalysis.dateWindow=(await analysisApi('dates',{analysisId:imported.analysisId,positions:state.dashboardPositions,date:state.dashboardDate})).page;
  ...
  function buildSummaryReportHtml(model, generatedAt = new Date()) {
    ...
    const palette = ['#2878b8','#11a095','#8664c0','#d79932'];
    const colorFor = position=>palette[Math.max(0,positions.findIndex(p=>p.position===position))%palette.length];
    const reportFiles=[];
  ...
  function currentAnalysisDashboardData() {
    ...
    const allRows = positionNames().filter(p=>state.dashboardPositions===null||state.dashboardPositions.includes(p)).flatMap(position => state.resultInputs[position]?.rows || []);
  ...
  async function downloadSelectedResults(position='',tool='') {
    if(state.remoteAnalysis){
      if(state.simulationAgent.analysisApiVersion<2)return showToast('선택 CSV 저장에는 새 Agent 업데이트가 필요합니다.',true);
      return exportAgentAnalysis(tool?'tool-ng':'selection',position,{tool,positions:state.dashboardPositions});
    }
    const records=(state.dashboardModel?.records||[]).filter(record=>tool
      ?record.position===position&&record.tools[tool]?.result==='NG'
      :state.dashboardPositions==null||state.dashboardPositions.includes(record.position));
    const tools=state.model?.tools||[];
    const headers=['Date','Time','Cell ID','Position','Total_result','Source_Row_Count'];
    tools.forEach(name=>headers.push(`${name}_result`,`${name}_score`,`${name}_threshold`));
    const lines=[headers.map(csvCell).join(',')];
    for(const record of records){
      const row=[dashboardDateForRecord(record),'',record.cellId,record.position,record.totalResult,record.sourceRows?.length||1];
    ...
    $$('[data-history-field]', shell).forEach((input) => {
      ...
      input.onchange = () => {
        ...
        if (['position','workspaceType','workspaceKey'].includes(field)) {
          if (field === 'position') { state.historyFilters.workspaceType = ''; state.historyFilters.positions=null; }
  ...
  function historyCountBarsLegacy(daily) {
    ...
    return `<div class="vq43-history-bars">${rows.map((row) => {
      const total = Number(row.total || 0), ng = Number(row.ng || 0);
      const ngHeight = Math.max(2, Math.round(ng / max * 100));
      const totalHeight = Math.max(4, Math.round(total / max * 100));
      return `<button type="button" class="vq43-history-bar" title="${escapeHtml(row.date)} · 전체 ${numberText(total)} · NG ${numberText(ng)} (${rateText(row.ngRate)})" data-vq-action="history-day" data-vq-history-day="${escapeHtml(row.date)}"><i style="height:${totalHeight}%"></i><b style="height:${ngHeight}%"></b><span>${escapeHtml(String(row.date || '').slice(5))}</span></button>`;
    }).join('')}</div>`;
  }
  ...
  function changeHistoryChartWindow(scope, action, date, step = 10) {
    if(scope==='main' && state.remoteAnalysis){
      ...
      analysisApi('dates',{analysisId:remote.analysisId,positions:state.dashboardPositions,date:state.dashboardDate,dateStart:action==='history-chart-latest'||(action==='date'&&!date)?-1:Math.max(0,start),dateAnchor:action==='date'?date:''}).then(response=>{
    ...
    if (action === 'history-chart-latest' || (action === 'date' && !date)) view.anchor = null;
      ...
      const next = Math.max(0, Math.min(Math.max(0,view.rows.length-10), view.start + (action === 'history-chart-prev' ? -step : step)));
  ...
  function renderHistory() {
    ...
    const chart = '<section class="vq43-section vq43-history-chart"><div class="vq43-section-title"><div><h3>날짜별 NG율</h3><p>촬영 시각이 없으면 검사 시각을 사용합니다. 세로축은 최고 NG율에 3%p 여유를 더해 자동 확대되며, 각 점에 NG율을 직접 표시합니다.</p></div></div>' + chartPositionControls('history') + historyDateBars(data.daily) + '</section>';
  ...
  function changeHistoryPage(page) {
    const total = Number(state.historyData?.totalCount || 0);
    const last = Math.max(1, Math.ceil(total / Number(state.historyFilters.pageSize || 50)));
    state.historyFilters.page = Math.max(1, Math.min(Number(page || 1), last));
    refreshHistory(false);
  }

  async function chooseHistoryCsvImport() {
    if (state.simulationAgent.status !== 'connected') return showToast('대용량 CSV 저장에는 실행 중인 Local Agent가 필요합니다.', true);
    try {
      ...
      if (state.page === 'history') renderHistory();
      pollHistoryFileImport(started.jobId);
    } catch (error) { showToast(`대용량 CSV 저장 실패: ${error.message || error}`, true); }
  }

  async function pollHistoryFileImport(jobId) {
    while (jobId) {
      await new Promise((resolve) => setTimeout(resolve, 750));
      try {
        const status = await agentFetch('/api/history/import-file/status', { method:'POST', timeout:8000, body:{ jobId } });
  ...
  function simulationDefaults() {
    ...
    return {
      ...
      green:{
        ...
        heatmapImageSave:true, forceJet:true, printEvery:100, originalProcess:false, disableTensorRt:false, freshRuntime:false, detailedDiagnostics:false, disableOptimizedGpuMemory:false,
  ...
  function ensureSimulationForm() {
    ...
    Object.assign(form.green, { originalProcess:false, disableTensorRt:false, freshRuntime:false,
  ...
  function syncSimulationField(input, allowStructuralRender = true) {
    ...
    } else if (allowStructuralRender && scope === 'green' && field === 'originalProcess') refreshSimulationOptionsOnly();
  ...
  function applySimulationLockDom() {
    ...
    const vpdlMissing = state.simulationAgent?.vpdlAvailable === false;
    const load = $('#vq43-runtime-file-load'); if (load) load.disabled = running || loading || state.simulationAgent.status !== 'connected' || vpdlMissing;
    const start = $('#vq43-sim-start'); if (start) start.disabled = running || loading || state.simulationAgent.status !== 'connected' || vpdlMissing;
  ...
  function markSimulationAgentOffline(message = 'Local Agent가 중지되었거나 통신할 수 없습니다.') {
    ...
    state.simulationAgent = { status:'offline', version:'-', vpdlAvailable:null, vpdl:'-', license:'-', gpu:'-', instanceId:'', message };
  ...
  async function pollSimulationAgentStatus() {
    ...
    try {
      ...
      const vpdlAvailable = data.vpdlAvailable !== false;
      ...
      if (!wasConnected || restarted) {
        ...
        if (vpdlAvailable) checkSimulationRuntime({ silent:true, reason:'agent-start' });
  ...
  async function checkSimulationRuntime({ silent=false, reason='automatic' } = {}) {
    if (state.simulationAgent?.vpdlAvailable === false)
      return { ok:false, vpdlAvailable:false, error:'VPDL이 설치되어 있지 않아 Runtime 기능을 사용할 수 없습니다.' };
    ...
    const task = (async () => {
      ...
      try {
        ...
        state.simulationAgent.message = data.ok ? (data.message || (data.preloaded ? 'Runtime 로드 완료 · 검사 시작 가능' : data.busy ? '검사 엔진 사용 중' : 'Agent 연결 확인 완료')) : (data.error || 'Runtime 확인 실패');
  ...
  async function startSimulation() {
    ...
    try {
      ...
      if (request.mode === 'green' && (request.green.originalProcess || request.green.disableTensorRt || request.green.freshRuntime || request.green.detailedDiagnostics || request.green.disableOptimizedGpuMemory) && state.simulationAgent.version !== EXPECTED_AGENT_VERSION) {
  ...
  function updateRuntimeLoadButtonDom() {
    ...
    button.textContent = state.simulationWorkspaceLoading
      ...
      : 'Runtime File Load';
    button.disabled = state.simulationWorkspaceLoading || !!state.simulationProgress?.running || state.simulationAgent.status !== 'connected' || state.simulationAgent.vpdlAvailable === false;
  }
  ...
  async function loadSelectedRuntimeFiles() {
    ...
    if (state.simulationAgent.vpdlAvailable === false) { showToast('VPDL이 설치되어 있지 않아 Runtime File Load를 사용할 수 없습니다.', true); return; }
  ...
  function simulationWorkspaceInspectorPanel() {
    ...
    const loadButton = `<button id="vq43-runtime-file-load" class="vq43-btn vq43-btn-blue" data-vq-action="simulation-runtime-load" ${state.simulationWorkspaceLoading || state.simulationAgent.status !== 'connected' || state.simulationAgent.vpdlAvailable === false ? 'disabled' : ''}>${buttonText}</button>`;
    if (!revealStructure) {
  ...
  function simulationAgentCardHtml() {
    ...
    const vpdlMissing = connected && agent.vpdlAvailable === false;
    const checking = agent.status === 'checking';
    const statusClass = vpdlMissing ? 'checking' : connected ? 'ready' : checking ? 'checking' : 'offline';
    const statusText = vpdlMissing ? 'Connected · VPDL 없음' : connected ? 'Connected' : checking ? 'Checking...' : 'Stopped';
    const runtimeTitle = vpdlMissing ? 'VPDL이 없어도 분석·검사 이력·이미지 조회 및 분류 기능은 사용할 수 있습니다.' : agent.runtimePreloaded || state.simulationProgress?.running
    ...
    const workerSelector = connected && !vpdlMissing && workers.length
  ...
  function installDebugApi() {
    ...
    window.__VISIONQC_DEBUG__ = {
      ...
      async syncAgentLiveTest(processed,final=false,mode='green'){
        state.simulationAgent={...state.simulationAgent,status:'connected',version:EXPECTED_AGENT_VERSION,analysisApiVersion:2};
      ...
      seedApprovedReport() {
        this.seedReportMetadata();
        state.ngImages=state.ngImages.filter((image,index)=>[1,2,101,301].includes(index));
        Object.values(state.resultInputs).forEach((input,pi)=>input.rows.forEach((row,i)=>{row.workspaceName=row.position.replace(/[()]/g,'_')+'_Model.vrws';if(i%(pi+3)===0){row.totalResult='NG';row.tools.Crack.result='NG';}}));
        rebuildModel();
      },
