...
- 병렬 CSV는 모든 분할 결과 통합 성공 후 목록 정리, 실패 시 재처리 목록 보존.
- 시간이 빈 집계 CSV 재입력 시 날짜가 사라지던 문제 수정 및 다중 CSV 내보내기/재입력 검증 추가.
- [상세 변경 내역](RELEASE_NOTES_v4.8.3_KR.md).
...
- 가져오기 전 형식·버전·클래스 ID·단축키 검증과 전체 슬롯 교체 확인. 오류·취소·저장 실패 시 기존 설정 보존.
- 하단 Save의 단축키 중복 및 입력 오류를 팝업으로 표시.
- 브라우저 프리셋 저장 실패를 안내하고 JSON 백업 방법 제공.
- 사용 방법: [Class Presets 파일 백업과 복원](CLASS_PRESETS_BACKUP_KR.md).
- 대용량 분석 전환, Agent와 오프라인 패키지의 4.8.1 배포는 별도 검증 진행 중.
...
# VisionQC 4.7.48 / Agent 1.3.35

- PDF 리포트 두 번째 구간의 반복 제목과 정보 영역 제거. 맨 위에 한 번만 표시.
- Position별 결과, 날짜 그래프, Tool Threshold, 미검 목록은 기존 구성 유지.
...
# 4.7.46 / Agent 1.3.33
- 요약 리포트 가독성/메타데이터 개선, 날짜별 그래프/Threshold 추가, 순번 없는 미검 목록 및 복사.

# 4.7.45 / Agent 1.3.32
- 날짜 그래프 끝 구간 10개 채움 및 호버 화살표로 5개 이동.

# 4.7.44 / Agent 1.3.31
- 날짜별 NG율 축 고정 및 날짜 선택/10개 단위 탐색. 최신 10개 기본 표시, 90개 제한 제거.
- Class Settings 폭과 중첩 여백 축소, 열 정렬 및 작은 화면 배치 개선.

# 4.7.43 / Agent 1.3.30
- 이미지 분류 라벨 해제 단축키 사용자 설정 및 충돌 검증 추가.

## 4.7.42 / Agent 1.3.29

설치 후 로컬 웹 창 자동 실행 제거, 완료 저장 팝업 전체 화면 표시, No 이후 대시보드 상단에서 DB 저장 지원.

## 4.7.41 / Agent 1.3.28

완료 후 DB 저장 선택, 경과 시간 종료 처리, Position별 이력 필터 연동, 날짜 표시/달력 클릭 개선, 라벨별 Cell ID XLSX 내보내기. 상세 검증은 RELEASE_NOTES_v4.7.41_KR.md 참고.

# v4.7.39 / Agent v1.3.26

- 날짜 NG율 그래프 글자 확대, 10개 고정 간격 및 중앙 정렬, 좌우 스크롤.
- 실제 미검 Cell의 최종 판정으로 Tool Score 범위 정합성 수정.

# VisionQC 변경 이력

## v4.7.38 / Agent 1.3.25

- 전체보기에서 서로 다른 날짜의 동일 Cell ID·Position을 하나로 합치던 집계 오류를 수정했습니다.
- 전체보기, 날짜 선택, 실시간 증분 집계가 모두 `날짜 + Cell ID + Position` 기준으로 같은 수치를 사용합니다.
- 검사 이력 DB의 화면 중복 제거에도 날짜를 포함해 과거 날짜가 최신 날짜에 의해 사라지지 않도록 수정했습니다.
- 미검 및 전체 결과 CSV에 촬영 시각과 원본 식별 열을 포함해 날짜별 검사 건을 구분할 수 있게 했습니다.
- 상세 검증·한계: `RELEASE_NOTES_v4.7.38_KR.md`.

## v4.7.37 / Agent 1.3.24

- Simulation 중 진행률 표시와 무거운 대시보드 전체 갱신을 분리해 화면이 주기적으로 멈추는 현상을 줄였습니다.
- 대시보드 전체 갱신은 누적 결과 수에 따라 3~12초 간격으로 자동 조절하며, 실행 완료 시에는 즉시 최종 화면을 표시합니다.
- 날짜별 NG율도 새 행과 중복 Cell·Position만 증분 반영해 이전 누적 행을 반복 계산하지 않습니다.
- 상세 검증·한계: `RELEASE_NOTES_v4.7.37_KR.md`.

## v4.7.36 / Agent 1.3.23

- 실행 중 대시보드가 누적 전체 행을 다시 계산하지 않고 새 SQLite 결과의 Cell·Position·Tool 통계만 증분 갱신합니다.
- 동일 Cell·Position 중복행은 기존 기여분을 제거하고 합친 결과를 다시 반영해 전체 재계산과 같은 판정을 유지합니다.
- 완료 시 Agent 처리 수, DB 저장 수, 전체 조회 수와 화면 변환 수가 모두 일치해야 전체 결과로 교체합니다.
- 상세 검증·한계: `RELEASE_NOTES_v4.7.36_KR.md`.

## v4.7.35 / Agent 1.3.22

- 실행 중 약 2초마다 현재 실행의 새 SQLite 결과만 읽어 대시보드 처리 건수를 따라가도록 개선했습니다.
- DB 증분 동기화 중 SSE 상세 행을 중복 집계하지 않으며 완료 후 전체 결과 재검증을 유지합니다.
- 상세 검증·한계: `RELEASE_NOTES_v4.7.35_KR.md`.
...
- 표시 범위 상단은 날짜별 최고 NG율에 3%p를 더한 값이며, 100%를 넘지 않음.
- 최고 NG율이 5%이면 0~8% 축으로 표시하여 낮은 구간의 변화를 쉽게 구분.
- 집계, 날짜 선택, 필터, Tool·Position·Workspace 판정 방식은 변경하지 않음.
- 상세 검증: `RELEASE_NOTES_v4.7.30_KR.md`.
...
- 외부 AI/API 키 의존을 제거하고 로컬 검사 호출, 완료 후 런타임 재준비, 오류 결과의 OK 오분류 차단.
- Green 태그 판정 유지, Red의 Regions/Score/처리 오류를 별도로 해석.
- 날짜 형식 3종과 위치 지정 전용 토큰 입력, 이름별 규칙 저장/불러오기.
- 파일명 촬영 날짜를 그래프에 그대로 표시, 날짜 클릭 시 대시보드 전체 필터, 전체보기 복원.
- 비교진단 실행기/자기 소유 바로가기/실험 옵션 정리. 기존 데이터와 개발 회귀 테스트 보존.
- 상세 검증·한계: RELEASE_NOTES_v4.7.25_KR.md.
...
- SDK 버전 선택, 진행률·SQLite·CSV·원본/처리/Overlay 경로를 유지. 독립 실행 시 TensorRT·메모리 덮어쓰기 없음.
- 프로세스 종료 시 오류 메시지 보존, 자신의 검사 프로세스만 중지. 다른 VPDL 학습 프로세스는 건드리지 않음.
- 원본 ZIP 엔진 비교, 4.0·4.2 네이티브 검사, 120 코드·29 브라우저·62 C# 검증. 보안 H100 서버 해결은 현장 확인 필요.
- 상세 절차: RELEASE_NOTES_v4.7.22_KR.md.
...
- Workspace의 로드 대기·읽는 중·로드 완료·Tool 상태를 밝은 표면과 어두운 텍스트로 정리한다.
- Simulation Options의 주요 글자 크기를 한 단계 올리고, 화이트 모드의 기본 텍스트 명암을 높인다.
- AI SUGGEST 결과 패널은 화면을 가리지 않는 좌측 하단의 짧은 요약 카드로 축소한다.
...
- CSV FullPath·SQLite 실제 NG 이미지 팝업과 확대 Score 차트 팝업은 `#vq43-shell` 바깥에서 표시되는 구조여서 기존 화이트 테마가 적용되지 않았습니다. 헤더·탭·경로·점수·조작 버튼·차트 안내를 밝은 하늘색과 어두운 텍스트로 통일했습니다.
- 이미지 표시 캔버스만은 원본의 여백과 명암 대비 확인을 위해 어두운 배경을 유지합니다.
- 메인 실제 NG 최소 Score는 분석 화면과 공통 계산 함수로 통일되어 있으며, Crack `0.7495` / Welding `0.5071` / 제외 기준 `0.70` 회귀 검증을 계속 유지합니다.

---

# VisionQC v4.7.5

- 메인 `Position별 Tool NG 구성`의 실제 NG 최소 Score도 분석 화면과 동일하게 다른 Tool NG 제외 기준을 적용한다.
- 다른 Tool NG 제외 기준 입력을 바꾸면 메인 집계도 즉시 다시 계산한다.
- 화이트 모드는 빈 화면, 메인 표·카드, 시뮬레이션의 옵션·진행 상태·표, 이미지 분류의 헤더·작업 영역·파일 탐색기까지 밝은 하늘색 계열과 어두운 텍스트로 통일했다.
...
- 저장된 CSV/XLSX 결과를 다시 불러올 때 이전에 선택한 대시보드 날짜 필터를 해제하도록 수정했습니다.
- 결과 파일 교체·제거·전체 초기화 뒤 메인 대시보드를 현재 입력 전체로 다시 집계합니다.
