...
namespace VisionQC.LocalAgent
{
    internal sealed class AgentServer : IDisposable
    {
        ...
        private object CancelPicker(string body)
        {
            ...
            AppendAgentLog(cancelled ? "WARN" : "INFO", cancelled
                ? "응답이 끝나지 않은 파일/폴더 선택 창을 닫았습니다."
        ...
        private BlueCropConfig BuildBlueConfig(AgentStartRequest req, string outputRoot, bool integrated)
        {
            ...
            cfg.Slots = EnabledPositions(req).Select(p => new BlueWorkspaceSlotConfig
            {
                ...
                ImageRoot = integrated && iopt.keywordMode ? iopt.keywordInputRoot : FirstNonEmpty(p.blueImageRoot, p.imageRoot),
        ...
        private AppConfig BuildGreenConfig(AgentStartRequest req, string outputRoot, string cropRoot, bool integrated)
        {
            ...
            cfg.WorkspaceSlots = EnabledPositions(req).Select(p => new WorkspaceSlotConfig
            {
                ...
                InputRoot = integrated ? Path.Combine(cropRoot, p.displayName) : (opt.keywordMode ? opt.keywordInputRoot : FirstNonEmpty(p.greenImageRoot, p.imageRoot)),
        ...
        private string ValidateRequest(AgentStartRequest req)
        {
            ...
            foreach (var p in positions)
            {
                if (mode == "green")
                {
                    ...
                    if (!g.keywordMode)
                    {
                        string root = FirstNonEmpty(p.greenImageRoot, p.imageRoot);
                        if (string.IsNullOrWhiteSpace(root) || !Directory.Exists(root)) return p.displayName + " Green Image Folder를 확인하세요.";
                    }
                }
                else if (mode == "blue")
                {
                    string ws = FirstNonEmpty(p.blueWorkspacePath, p.workspacePath);
                    string root = FirstNonEmpty(p.blueImageRoot, p.imageRoot);
                    if (string.IsNullOrWhiteSpace(ws) || !File.Exists(ws)) return p.displayName + " Blue Workspace를 확인하세요.";
                    if (string.IsNullOrWhiteSpace(root) || !Directory.Exists(root)) return p.displayName + " Blue Image Folder를 확인하세요.";
                }
                else
                {
