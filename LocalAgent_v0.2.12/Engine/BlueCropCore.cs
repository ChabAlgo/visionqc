...
namespace VpdlGreenHeatmapOverlay
{
    ...
    internal class BlueWorkspaceSlotConfig
    {
        ...
        public List<string> ImageRoots { get; set; } = new List<string>();
    ...
    internal static class BlueCropProcessor
    {
        ...
        {
            List<string> roots = GetImageRoots(slot);
            string root = roots.FirstOrDefault(candidate => IsUnderDirectory(sourceFullPath, NormalizeDir(candidate))) ?? slot.ImageRoot;
            string rel = GetRelativePath(root, sourceFullPath);
            string dir = config.KeepSubfolders ? (Path.GetDirectoryName(rel) ?? "") : "";
            if (roots.Count > 1)
            {
                int sourceIndex = roots.FindIndex(candidate => string.Equals(candidate, root, StringComparison.OrdinalIgnoreCase));
                string sourceName = Path.GetFileName(root.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar));
                if (string.IsNullOrWhiteSpace(sourceName)) sourceName = "Root";
                foreach (char invalid in Path.GetInvalidFileNameChars()) sourceName = sourceName.Replace(invalid, '_');
                dir = Path.Combine(string.Format("Source_{0:D2}_{1}", Math.Max(0, sourceIndex) + 1, sourceName), dir);
            }
        ...
        private static List<string> GetImageRoots(BlueWorkspaceSlotConfig slot)
        {
            ...
            if (slot != null && slot.ImageRoots != null) foreach (string root in slot.ImageRoots) add(root);
            if (slot != null) add(slot.ImageRoot);
            return roots;
        }
