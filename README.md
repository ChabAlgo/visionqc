# VisionQC v4.7.22 / Local Agent v1.3.12

## 현재 릴리스 안내

- 설치 파일: [VisionQC Agent 1.3.12](downloads/VisionQC_Agent_Installer_v1.3.12.exe)
- 오프라인 패키지: [VisionQC Offline 4.7.22](downloads/VisionQC_Offline_v4.7.22.zip)
- SQLite 이력, 대용량 CSV 직접 적재, 파일명 Position 기반 AI Green 검사, 원본/Heatmap Viewer를 제공합니다.
- 최신 변경·검증·보안 서버 확인: [v4.7.22 릴리스 노트](RELEASE_NOTES_v4.7.22_KR.md). 기존 설계는 `ARCHITECTURE_V4.7.0_KR.md` 참고.
- 소스 폴더 `LocalAgent_v0.2.12`는 유지된 경로명입니다. 실제 배포 Agent 버전은 1.3.12이며 `RELEASE_MANIFEST.json`과 EXE 파일 버전으로 확인합니다.

- 이번 배포: 원본 DL_Simulation의 Runtime 생성·검사 수명주기를 따르는 독립 Green 실행 프로세스. 기존 분석·분류·CSV·SQLite 기능은 유지합니다. H100 현장 오류 해결은 아직 확정되지 않았습니다.
